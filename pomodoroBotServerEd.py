...
def setup2(message):
    if '-' in message.text:
        ...
        bot.reply_to(message,'Sorry but you insert an invalid format.'+'\n'+'Please try again!')

def setup3(message):
    if message.text.isalpha() == False and '-' not in message.text:
        userID = str(message.from_user.id)
        bot.settings[userID]['pomodoro'] = int(message.text)
        bot.reply_to(message,fr'Alright then, '+"🍅"*int(bot.settings[userID]['pomodoro'])+" ready to be crushed!")
    else:
...
def pummarolaWorking(message):
    ...
    if userID not in bot.settings.keys():
        ...
        bot.send_message(message.chat.id, fr"Take a look to your progress here /progress")
        while workTime > 0:
        ...
        bot.buffers.clear()
        markup = types.ForceReply(selective=False)
        res = bot.send_message(message.chat.id, fr"Good job! You've finished one session of {bot.settings[userID]['wTime']} min."+"\n"+"When you are ready to relax send me something!", reply_markup=markup)
...
def pummarolaResting(message):
    ...
    timer = dict()
    bot.buffers[userID] = timer
    bot.send_message(message.chat.id, fr"Now take a glass of water and relax for {bot.settings[userID]['rTime']} min.!"+"\n"+"See how much you have left from /progress")
    while restTime > 0:
        ...
        time.sleep(1)
        restTime-=1
    bot.settings[userID]['pomodoro'] = int(bot.settings[userID]['pomodoro']) - 1
    bot.buffers.clear()
    if int(bot.settings[userID]['pomodoro']) == 0:
        bot.send_message(message.chat.id, fr"Good Job buddy! You did a terrific pummarola today!")
    else:
        markup = types.ForceReply(selective=False)
...
def progress(message):
    userID = str(message.from_user.id)
    bot.reply_to(message, fr"You're doing great, only {bot.buffers[userID]['mins']} minutes and {bot.buffers[userID]['secs']} seconds left!"+"\n"+"-> /progress")
...
def webhook():
    bot.remove_webhook()
    bot.set_webhook(url='https://pomodoro-telegram-bot.herokuapp.com/' + token)
    return "!", 200
